...
<template>
  ...
  <div class="filter-group" data-label="가입 기간">
    ...
    <div class="table-content">
      <SavingListItem 
        v-for="(saving, index) in displayedSavings" 
        :key="saving.id"
        :saving="saving"
        :rank="index + 1"
      />
...
// 필터링 및 정렬
const sortedAndFilteredSavings = computed(() => {
  const filtered = store.savings.filter(saving => {
    const termMatch = selectedTerm.value === null || saving.save_trm === selectedTerm.value
    const rateTypeMatch = selectedRateType.value === null || saving.intr_rate_type_nm === selectedRateType.value
    const bankTypeMatch = selectedBankType.value === null || saving.fin_grp_cd === selectedBankType.value
...
const displayedSavings = computed(() => {
  return sortedAndFilteredSavings.value.slice(0, page.value * itemsPerPage)
})
...
const handleScroll = () => {
  ...
  if (scrollPosition >= documentHeight - 100) {
    if (displayedSavings.value.length < sortedAndFilteredSavings.value.length) {
      loadMoreData()
    }
  }
}
