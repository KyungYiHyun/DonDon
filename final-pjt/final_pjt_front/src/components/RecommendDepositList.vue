<template>
  <div class="container">
    <!-- Top 3 Cards -->
    <div class="row mb-5">
      <div v-for="(product, index) in depositTopThree" :key="index" class="col-md-4 cursor-pointer" data-bs-toggle="modal" :data-bs-target="`#depositModal-${product.id}`">
    ...
    <!-- <div class="table-responsive">
      ...
      <table class="table table-hover">
        ...
        <tbody>
          <RecommendDepositListItem 
            v-for="(deposit,index) in store.recommended_deposits.slice(3)" 
            :key="deposit.id" 
            :deposit="deposit"
            :rank ="index + 1"
          />
    ...
    <!-- Modal -->
    <div v-for="product in depositTopThree" :key="product.id">
      <div 
        class="modal fade custom-modal"
        :id="`depositModal-${product.id}`"
        tabindex="-1"
...
<script setup>
import RecommendDepositListItem from './RecommendDepositListItem.vue';
...
const depositTopThree = ref([])
const selectedProduct = ref(null)


onMounted(()=>{
  store.recommendDeposits()
})
watch(
  () => store.recommended_deposits,
  (newValue) => {
    depositTopThree.value = newValue.slice(0,3)
...
const openModal = (index) => {
  selectedProduct.value = depositTopThree.value[index]
