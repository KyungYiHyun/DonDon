            ...
            <div class="info-card">
              <div class="card-body">
                ...
                <div class="info-item">
                  ...
                  <span v-if="product.join_deny == 3"  class="info-value">일부제한</span>
                  <span v-if="product.join_deny == 2" class="info-value">서민전용</span>
                  <span v-if="product.join_deny == 1" class="info-value">제한없음</span>
                </div>
              </div>
            </div>
          </div>
  ...
  <span class="info-value">{{ product.max_limit ? `${product.max_limit} 원` : '없음' }}</span>
    ...
    <div class="modal-footer">
      <button 
        ...
        @click="isInCart(product.id) ? deleteFromDepositCart(product) : addToDepositCart(product)"
      >
...
const addToDepositCart = (product) => {
  store.addToDepositCart(product)
}

const deleteFromDepositCart = (product) => {
  store.deleteFromDepositCart(product)
}

const isInCart = (productId) => {
  return store.depositProducts.some((item) => item.id === productId)
}
