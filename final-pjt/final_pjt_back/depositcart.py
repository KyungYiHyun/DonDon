...
with open("depositproducts.json", "r", encoding="utf-8") as file:
    deposit_products = json.load(file)

# Filter products with save_trm >= 6
filtered_products = [
    product for product in deposit_products
    if product["fields"]["save_trm"] >= 6
]
...
for user_id in user_ids:
    ...
    for product in selected_products:
        user_to_product_mapping.append({
            "model": "fin.depositproducts_like_users",
            "pk": len(user_to_product_mapping) + 1,
            "fields": {
                "user_id": user_id,
                "depositproducts_id": product["pk"]
            }
        })

# Save the mappings to a JSON file
with open("depositproducts_like_users.json", "w", encoding="utf-8") as output_file:
